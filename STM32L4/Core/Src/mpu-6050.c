...
void MPU_6050_Self_Test(MPU6050_t *handles) {
    ...
    uint8_t gyro_config = GYRO_CONFIG_VAL;
    uint8_t accel_config = ACCEL_CONFIG_VAL;
    ...
    selftest_results.accel_x = selftest_ratio(diff.accel_x, ft.accel_x);
    selftest_results.accel_y = selftest_ratio(diff.accel_y, ft.accel_y);
    selftest_results.accel_z = selftest_ratio(diff.accel_z, ft.accel_z);
